...
# plot
def comp_plot(model_str, x_test, compression_ratios, snr_lst, title, x_lablel, y_label):
...
# plot
def snr_plot(model_str, x_test, compression_ratio, snr_train, title, x_lablel, y_label):
    markers = ["*", "s", "o", "X", "d", "v", "<", ">", "^", "P", "H", "|"]
    colors = list(mcolors.TABLEAU_COLORS)
    history = []
    for model in model_str:
        ...
        for snr in snr_train:
            print('\n----> Now Getting Data and Preparing Plot for SNR {0} dB <----'.format(snr))
            model_dic = snr_eval(model, x_test, compression_ratio, snr_test, snr)
            history.append(model_dic)
